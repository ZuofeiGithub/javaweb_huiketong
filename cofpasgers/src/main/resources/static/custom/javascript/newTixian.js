...
layui.use(['form', 'table'], function(){
    ...
    table.render({
        ...
        ,cols: [
            [
                {field:'drawalTime', title:'提现时间', width:'20%',templet: "<div>{{ layui.util.toDateString(d.drawalTime,'yyyy-MM-dd')}}</div>"}
                ,{field:'angentname', title:'经纪人', width:'15%'}
                ,{field:'money', title:'提现金额(元)', width:'15%'}
                ,{field:'status', title:'状态', width:'15%',templet: '#statusTpl'}
                ,{field:'updateTime', title:'操作时间', width:'20%',templet: "<div>{{ layui.util.toDateString(d.updateTime,'yyyy-MM-dd')}}</div>"}
